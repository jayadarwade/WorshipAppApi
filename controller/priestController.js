...
exports.priestRegistration = async (req, res) => {
    ...
    console.log(name + " " + email + " " + password + " " + password_confirmation + " " + mobile + " " + age + " ")
    const image = "https://firebasestorage.googleapis.com/v0/b/worship-first.appspot.com/o/" + req.file.filename + "?alt=media&token=hello"
    ...
    // } else {
    if (name && email && password && password_confirmation && mobile && age && image && about) {
        if (password === password_confirmation) {
            try {
                ...
                await doc.save()
                uploadFile(path.join(__dirname, "../", "public/images/") + req.file.filename);
...
exports.priestUpdateprofile = (request, response) => {
    ...
    priestModel.updateOne({ _id: request.body.id },
        {
            $set: {
    ...
    try {
        ...
        if (password && password_confirmation) {
            if (password !== password_confirmation) {
                ...
                const user = await priestModel.findById(id)
                await priestModel.findByIdAndUpdate(user._id, { $set: { password: newHashPassword } })
...
exports.delete= (req,res)=>{
    priestModel.deleteOne({_id:req.params.id}).then(result=>{
        return res.status(200).json(result);
    }).catch(err=>{
        console.log(err);
        return res.status(500).json({ message: "something went wrong" })
