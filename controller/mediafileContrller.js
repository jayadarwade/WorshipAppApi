const mediafile = require("../model/mediafileModel");
  ...
  mediafile
    ...
    .then((result) => {
      ...
      if (result.deletedCount)
        return response.status(202).json({ status: "success full delete" });
      else return response.status(204).json({ message: "not delete" });
    })
    .catch((err) => {
      console.log(err);
...
exports.update=(request,response)=>{
  ...
  .then(result=>{
    ...
    if(result.modifiedCount >0)
      ...
      return response.status(200).json({message:'not update'})
  }).catch(err=>{
    console.log(err)
      return response.status(500).json({message:'failed'});
  })
